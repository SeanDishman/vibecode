...
public sealed class TurnRollbackCheckpoint
{
    ...
    private static readonly HashSet<string> IgnoredDirectories = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // does not edit these, but their bytes are counted against MaxSnapshotBytes like any other source, and a
        // backup shelf is usually far larger than the project it backs up - measured here at 293 MB of shelf against
        // 16 MB of real project, which by itself failed every capture and disabled rewind. Only the "_"/"." prefixed
        // forms are listed: a bare "archive" or "backup" is plausibly a real source folder in someone's project, and
        // excluding real source would silently narrow what undo can restore. Same trade as "dist" and "publish"
        // above - a file edited inside one of these is not restorable by rewind.
        "_archive", "_archives", "_backup", "_backups", "_old", "_other-sources",
        ".archive", ".backup", ".old",
    };
    ...
    private static bool IsIgnoredWorkspaceFile(string relativePath) =>
        ...
        || IgnoredFileExtensions.Contains(Path.GetExtension(relativePath));

    /// <summary>
    /// Compiled output that is not sitting inside a folder named bin/obj. A moved-out source shelf, an unzipped
    /// release, or a vendored runtime scatters .dll/.exe/.so through ordinary directories, so the bin-*/obj-* prune
    /// never sees them - and because these are the biggest files in a repo they are what actually exhausts
    /// <see cref="MaxSnapshotBytes"/>. Measured on this workspace: 488 MB of eligible bytes against a 256 MB budget,
    /// 96% of it binaries under two shelf folders. That threw on EVERY capture, and a failed capture sets
    /// _failureReason, which disables the rewind button - so undo was not "flaky" here, it was dead in this project
    /// and fine in a small one. These bytes are regenerated by building and are never hand-edited, which is the same
    /// reasoning the directory prune already encodes; this just applies it to the files that escaped a pruned name.
    /// </summary>
        ...
        if (before is not null && UnavailableReason is null)
        {
            try
            {
                ...
                foreach (var path in touched)
                {
                    ...
                    // The tree scan leaves oversized files hash-less. One the turn actually touched still needs a real
                    // hash so its undo journal can verify it later; that cost is bounded to reported files, not the
                    // whole workspace. Paths oversized *before* the turn already threw via _uncapturedExisting above.
                    if (current is { Hash: null }) current = ReadCurrent(path);
    ...
    private Dictionary<string, CapturedFile> CaptureWorkspace()
    {
        ...
        // Pass 1 walks the tree without opening a single file. A workspace that cannot be snapshotted is by far the
        // most expensive case to discover late: the old single-pass version read up to MaxSnapshotBytes of content and
        // then threw it all away, and that wasted I/O is what "preparing prompt…" sat on before every send. Settling
        // the budget on a stat-only walk turns an impossible workspace into a fast "rewind unavailable" instead.
        ...
        while (pending.Count > 0)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                {
                    _unrestorableBefore[full] = ReadOversizedState(full)
                        ?? throw new IOException($"{Relative(full)} disappeared during checkpoint capture.");
                    _uncapturedExisting.Add(full);
                    continue;
                }
                if (eligible.Count >= MaxSnapshotFiles)
        ...
        // Pass 2 reads the content the checkpoint actually keeps. The budget is re-checked as it goes because a file
        // can grow between the two passes; ReadCaptured independently rejects one that crosses MaxFileBytes.
        var result = new Dictionary<string, CapturedFile>(eligible.Count, StringComparer.OrdinalIgnoreCase);
        long capturedBytes = 0;
        foreach (var full in eligible)
        {
            var captured = ReadCaptured(full);
            capturedBytes += captured.Content.LongLength;
    ...
    private Dictionary<string, CurrentFile> ScanCurrentWorkspace()
    {
        ...
        while (pending.Count > 0)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                // Mirror the capture pass: oversized files are unrestorable either way, so reconciliation only needs
                // to know whether they moved - not what they contain.
                result[full] = (file.Length > MaxFileBytes ? ReadOversizedState(full) : ReadCurrent(full))
    ...
    private CurrentFile? ReadCurrent(string path)
    {
        ...
        for (var attempt = 0; attempt < 2; attempt++)
        {
            ...
            var after = new FileInfo(path);
            if (after.Exists && after.Length == length && after.LastWriteTimeUtc == written)
                return new(hash, length, attributes, before.CreationTimeUtc, before.LastWriteTimeUtc);
        }
        throw new IOException($"{Relative(path)} changed while it was being checked.");
    }

    /// <summary>
    /// Record an oversized file's identity without reading it. Files past <see cref="MaxFileBytes"/> are already
    /// tracked as unrestorable, so a checkpoint never needs their bytes - only whether they changed during the turn.
    /// Hashing them anyway made every prompt stream gigabytes of archives/installers off disk before the provider was
    /// even contacted (a real workspace here spent 55s hashing 5.6 GB), which is what "preparing prompt…" was waiting
    /// on. Length+LastWriteTimeUtc is the same freshness signal <see cref="ReadCaptured"/> and
    /// <see cref="ReadCurrent"/> already trust to detect a file mutating mid-read.
    /// </summary>
    private static CurrentFile? ReadOversizedState(string path)
    {
        var file = new FileInfo(path);
        if (!file.Exists) return null;
        return new(null, file.Length, file.Attributes, file.CreationTimeUtc, file.LastWriteTimeUtc);
    }
    ...
    private static bool SameState(CurrentFile? left, CurrentFile? right)
    {
        ...
        // Oversized entries carry no hash, so compare the identity recorded in its place. A file that crossed
        // MaxFileBytes during the turn has a hash on exactly one side and is correctly reported as changed.
        if (left.Hash is null || right.Hash is null)
            return left.Hash is null && right.Hash is null
    ...
    // Hash is null for files past MaxFileBytes: they are recorded as unrestorable, so their bytes are never needed -
    // only "did this change during the turn?", which Length+LastWriteTimeUtc answers without reading the file.
    private sealed record CurrentFile(string? Hash, long Length, FileAttributes Attributes,
        DateTime CreationTimeUtc, DateTime LastWriteTimeUtc);
